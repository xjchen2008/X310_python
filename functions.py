...
class ReadBin:
    def readbin2(self, filename, count, offset, dtype=np.short):
        bytespersample = 4
        x = np.fromfile(filename, dtype, 2 * count, offset=bytespersample*offset)  # read the data into numpy
        real = x[::2] #/ 32768.0
        imag = x[1::2] #/ 32768.0
        rx_sig = real + 1j * imag
        return rx_sig

    def last_pulse(self, filename, nsamples, rubish,channel_num=1):
        fsize = int(os.stat(filename).st_size)
        bytespersample = 4 # for short
        samplesperpulse = nsamples
        total_samples = fsize / bytespersample
        total_pulse = total_samples / samplesperpulse
        offset = int((total_pulse-1)*samplesperpulse)
        signal = self.readbin2(filename=filename, count=nsamples, offset=offset)
        if channel_num ==1:
            return signal
        if channel_num ==2:
    ...
    def get_slice(self, data, pulselenth):
        data_ch0 = []
        data_ch1 = []
...
def gate(x, M,  n_pulse=20, n_gate=40, n_offset=0):
    n = len(x)
    #n_gate = 10
    #n_pulse = 20
    x_gated_concatenated = []
    for i in range(0, M):
        idx = n_offset + n_gate*i
        x_gated = x[ idx : idx + n_pulse]
        x_gated_concatenated = np.concatenate((x_gated_concatenated,x_gated))
    return x_gated_concatenated
...
def plot_freq_phase(freq, x, color='b'):
    # This function plots the input in frequency domain in dB
    X = np.fft.fft(x,axis = 0)
    X_phase = np.angle(X)
    plt.plot(fftshift(freq/1e6), fftshift(X_phase), color=color)
    plt.grid(b= True)
    plt.xlabel('Frequency [MHz]')
    plt.ylabel('Phase')
    return X
...
class Coe:
    def __init__(self, fc=50e6, bw=20e6, fs=200e6, N=1000):
