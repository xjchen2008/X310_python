...
def last_pulse(filename, nsamples, rubish,channel_num=1):
...
def read_last_pulse(filename, N, pulse_idx_from_back =0):
    ...
    data_ch0 = last_pulse(filename, nsamples * channels, -2*(pulse_idx_from_back+1)*N, channel_num=channels) #two channels; pulse_idx_from_back+1 because the last pulse could be not a good chirp, so use the second from the back
